...
<body>
    ...
    <header>
        <h2> Welcome Admin - Update Project Details</h2>
    </header>
        ...
        <div id="content">
            ...
            <form method="POST">
                <div style="display:flex;margin-top:10px;">
                    ...
                    <input type="text" id="weekNo" name="groupno" value="<?php echo $row['groupno']; ?>"
                        style="width:auto;margin-left:20px">
                </div>
                <label for="weekNo">Project Title:<span style="color:red;">*</span></label>
                <input type="text" id="weekNo" name="projectt" required value="<?php echo $row['projecttitle']; ?>">
                <label for="details">Project Details:<span style="color:red;">*</span></label>
                <textarea id="details" name="detailss" required><?php echo $row['projectdetail']; ?></textarea>
                ...
                <div id="additionalFields" style="display: none;">
                    ...
                    <input type="text" id="field1" name="sname" placeholder="no"
                        value="<?php echo $row['sponsershipname']; ?>" required>
                    <label for="field2">Sponsorship Details:</label>
                    <textarea id="field2" name="sdetails" required><?php echo $row['sponsershipdetail']; ?></textarea>
                </div>
...
if(isset($_POST['submit'])) {
    ...
    if($sponsorship === 'yes') {
        ...
        // If sponsorship is selected, update all details including sponsorship
    ...
    } else if($sponsorship === 'no'){
        // If sponsorship is not selected, update project title only

        $updateQuery = "UPDATE grouptable SET projecttitle='$projectt', projectdetail='$detailss', sponsershipdetail='',sponsershipname='' WHERE id='$g_id'";
    }
    ...
    // Check if there's any error in SQL execution
    if(!$updateResult) {
    ...
    if($updateResult) {
        echo "<script>";
        echo "window.alert('Project details updated successfully.');";
        echo "window.location.href = 'groupmanagement.php?year=$year';";
        echo "</script>";
    } else {
        // Display an error message if the update fails
        echo "<script>";
        echo "window.alert('Error updating project details.');";
        echo "</script>";
    }
}
?>